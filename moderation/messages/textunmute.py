    ...
    if not message.author.permissions_in(message.channel).manage_messages:
        ...
        if not message.mentions:
            ...
            if author.id == target.id and not is_admin:
                response = discord.Embed(title='❗ Can\'t unmute yourself.', color=0xDB0000)
            else:
                above_hier = hierarchy_permit(author, target)
                if not above_hier and not is_admin:
                    response = discord.Embed(title='⛔ Can\'t unmute someone equal or above you.', color=0xDB0000)
                else:
                    ...
                    if target.id not in mute_list:
                        ...
                        response = discord.Embed(color=0x66CC66, title=f'✅ {target.display_name} has been unmuted.')
